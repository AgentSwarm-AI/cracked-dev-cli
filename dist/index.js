...
import { autoInjectable as autoInjectable2 } from "tsyringe";
import { z } from "zod";
...
var configSchema = z.object({
  ...
  executeModel: z.string().optional().default("anthropic/claude-3.5-sonnet:beta"),
  autoScaleAvailableModels: z.array(z.object({
    id: z.string(),
  ...
  directoryScanner: z.object({
    defaultIgnore: z.array(z.string()).default([
      "dist",
      "coverage",
      ".next",
      "build",
...
`;
  ...
  createDefaultConfig() {
    if (!fs.existsSync(this.CONFIG_PATH)) {
      ...
      console.log(chalk.yellow("Warning: No OpenRouter API key provided. Please add it to crkdrc.json."));
      this.ensureGitIgnore();
      chalk.green("CrackedDevCLI config generated. Please, add Provider and API Key to crkdrc.json.");
    }
  }
  getConfig() {
    if (!fs.existsSync(this.CONFIG_PATH)) {
...
import { autoInjectable as autoInjectable3, inject } from "tsyringe";
...
var _DirectoryScanner = class _DirectoryScanner {
  ...
  async scan(dirPath, options = {}) {
    try {
      ...
      const absolutePath = path2.resolve(dirPath);
...
DirectoryScanner = _ts_decorate3([
  autoInjectable3(),
  _ts_param(0, inject(ConfigService)),
...
import { autoInjectable as autoInjectable30 } from "tsyringe";

// src/services/LLM/context/MessageContextHistory.ts
import { autoInjectable as autoInjectable6, singleton as singleton10 } from "tsyringe";

// src/services/LLM/PhaseManager.ts
import { injectable, singleton as singleton4 } from "tsyringe";
...
import { autoInjectable as autoInjectable4, singleton } from "tsyringe";
...
import { singleton as singleton3 } from "tsyringe";

// src/constants/openRouterClient.ts
import axios from "axios";
import { container } from "tsyringe";
var createOpenRouterClient = /* @__PURE__ */ __name((baseURL) => {
  const configService4 = container.resolve(ConfigService);
...
var configService = container2.resolve(ConfigService);
...
var configService2 = container3.resolve(ConfigService);
...
var configService3 = container4.resolve(ConfigService);
...
import { autoInjectable as autoInjectable5, singleton as singleton6 } from "tsyringe";

// src/services/LLM/context/MessageContextExtractor.ts
import { singleton as singleton5 } from "tsyringe";
...
import { inject as inject4, singleton as singleton9 } from "tsyringe";

// src/services/LLM/context/MessageContextStore.ts
import { delay as delay2, inject as inject3, singleton as singleton8 } from "tsyringe";

// src/services/LLM/context/MessageContextTokenCount.ts
import { encode } from "gpt-tokenizer";
import { delay, inject as inject2, singleton as singleton7 } from "tsyringe";
...
MessageContextTokenCount = _ts_decorate10([
  singleton7(),
  _ts_param2(0, inject2(delay(() => MessageContextStore))),
...
MessageContextStore = _ts_decorate11([
  singleton8(),
  _ts_param3(0, inject3(delay2(() => MessageContextTokenCount))),
...
MessageContextLogger = _ts_decorate12([
  singleton9(),
  _ts_param4(2, inject4(MessageContextBuilder)),
  _ts_param4(3, inject4(MessageContextStore)),
...
import { autoInjectable as autoInjectable7 } from "tsyringe";
...
import { autoInjectable as autoInjectable9 } from "tsyringe";

// src/services/LLM/actions/core/ActionFactory.ts
import { autoInjectable as autoInjectable8, container as container5 } from "tsyringe";
...
var _ActionFactory = class _ActionFactory {
  ...
  createAction(tag) {
    ...
    try {
      const instance = container5.resolve(blueprint.class);
...
import { autoInjectable as autoInjectable11 } from "tsyringe";
...
import { autoInjectable as autoInjectable12 } from "tsyringe";
    ...
    return new Promise((resolve2) => {
      ...
      child.stdout.on("data", (data) => {
        ...
        if (!this.isTestEnvironment()) {
          process.stdout.write(chalk2.green(chunk));
        }
      });
      child.stderr.on("data", (data) => {
        ...
        stderrBuffer = "";
        if (!this.isTestEnvironment()) {
          process.stderr.write(chalk2.red(chunk));
        }
      });
...
import { autoInjectable as autoInjectable13 } from "tsyringe";
...
var _FileSearch = class _FileSearch {
  ...
  async findByPattern(pattern, directory) {
    try {
      ...
      for (const entry of entries) {
        const content = await fs4.readFile(entry, "utf-8");
  ...
  async findByContent(searchContent, directory) {
    try {
      ...
      for (const entry of entries) {
        try {
          const stats = await fs4.stat(entry);
  ...
  async findByName(name, directory) {
    try {
      const targetName = path4.basename(name);
      const targetDir = path4.dirname(name);
      ...
      const fileEntries = entries.map((entry) => ({
        fullPath: entry,
        name: path4.basename(entry).toLowerCase(),
        dir: path4.dirname(entry)
      }));
...
import * as Fuse2 from "fuse.js";
import path5 from "path";
import { autoInjectable as autoInjectable14 } from "tsyringe";
...
var _PathAdjuster = class _PathAdjuster {
  ...
  async getAllFiles(dir) {
    try {
      const entries = await fg2.sync("**/*", {
        cwd: dir,
        absolute: true,
        onlyFiles: true,
        followSymbolicLinks: true
      });
      return entries.map((filePath) => path5.resolve(filePath));
    } catch (error) {
  ...
  findClosestMatch(wrongPath, threshold = 0.6) {
    ...
    const absoluteWrongPath = path5.resolve(this.baseDirectory, wrongPath);
  ...
  toRelativePath(absolutePath) {
    if (!path5.isAbsolute(absolutePath)) {
      const possiblePath = path5.join(this.baseDirectory, absolutePath);
    ...
    return path5.relative(this.baseDirectory, normalizedPath);
  }
...
import { autoInjectable as autoInjectable15 } from "tsyringe";
...
var _FileOperations = class _FileOperations {
  ...
  async adjustPath(filePath, isRead = false) {
    ...
    if (isRead) {
      const similarFiles = await this.fileSearch.findByName(path6.basename(filePath), process.cwd());
      if (similarFiles.length > 0) {
        const bestMatch = similarFiles[0];
        if (await fs5.pathExists(bestMatch)) {
          const stats = await fs5.stat(bestMatch);
          if (stats.isFile()) {
    ...
    const adjustedPath = await this.pathAdjuster.adjustPath(filePath);
    if (adjustedPath && await fs5.pathExists(adjustedPath)) {
  ...
  async read(filePath) {
    try {
      ...
      if (!await fs5.pathExists(adjustedPath)) {
        return {
          success: false,
          error: new Error(`File does not exist: ${filePath}`)
        };
      }
      const content = await fs5.readFile(adjustedPath, "utf-8");
      return {
        success: true,
        data: content
      };
...
${content}`);
  ...
  async write(filePath, content) {
    try {
      ...
      await fs5.ensureDir(path6.dirname(adjustedPath));
      await fs5.writeFile(adjustedPath, content);
      return {
        success: true
      };
    } catch (error) {
  ...
  async copy(source, destination) {
    try {
      ...
      if (!await fs5.pathExists(adjustedSource)) {
        return {
          success: false,
          error: new Error(`Source file does not exist: ${source}`)
        };
      }
      await fs5.ensureDir(path6.dirname(destination));
      await fs5.copy(adjustedSource, destination);
      return {
        success: true
      };
    } catch (error) {
  ...
  async move(source, destination) {
    try {
      ...
      await fs5.ensureDir(path6.dirname(destination));
      await fs5.move(adjustedSource, destination, {
        overwrite: true
      });
      return {
        success: true
  ...
  async exists(filePath) {
    const adjustedPath = await this.adjustPath(filePath);
    return fs5.pathExists(adjustedPath);
  }
  async stats(filePath) {
    try {
      ...
      const stats = await fs5.stat(adjustedPath);
      const fileStats = {
        size: stats.size,
...
import { autoInjectable as autoInjectable16 } from "tsyringe";
...
import { autoInjectable as autoInjectable17 } from "tsyringe";
...
import { autoInjectable as autoInjectable19 } from "tsyringe";
...
import { autoInjectable as autoInjectable18, singleton as singleton11 } from "tsyringe";
...
import { autoInjectable as autoInjectable20 } from "tsyringe";
...
import { autoInjectable as autoInjectable21 } from "tsyringe";
...
import { autoInjectable as autoInjectable22 } from "tsyringe";
...
import { autoInjectable as autoInjectable23 } from "tsyringe";
...
import { autoInjectable as autoInjectable24 } from "tsyringe";
...
import { autoInjectable as autoInjectable25 } from "tsyringe";
...
import { autoInjectable as autoInjectable26 } from "tsyringe";
...
import { autoInjectable as autoInjectable27 } from "tsyringe";
...
var _RelativePathLookupAction = class _RelativePathLookupAction extends BaseAction {
  ...
  async executeInternal(params) {
    try {
      ...
      const sourceDir = path7.dirname(source_path);
      const fullImportPath = path7.resolve(sourceDir, relativePath);
      const adjustedPath = await this.pathAdjuster.adjustPath(fullImportPath, threshold);
      if (adjustedPath) {
        const newRelativePath = path7.relative(sourceDir, adjustedPath);
...
import { autoInjectable as autoInjectable28 } from "tsyringe";
...
import { singleton as singleton12 } from "tsyringe";
...
import { autoInjectable as autoInjectable29 } from "tsyringe";
...
import { singleton as singleton13 } from "tsyringe";
...
import { autoInjectable as autoInjectable31 } from "tsyringe";
...
var _ProjectInfo = class _ProjectInfo {
  ...
  async gatherProjectInfo(projectRoot) {
    const files = await fs6.promises.readdir(projectRoot, {
      withFileTypes: true
    });
  ...
  async gatherNodeInfo(projectRoot, dependencyFile) {
    try {
      const packageJsonPath = path8.join(projectRoot, dependencyFile);
      const content = await fs6.promises.readFile(packageJsonPath, "utf-8");
  ...
  async gatherPythonInfo(projectRoot, dependencyFile) {
    try {
      let dependencies = [];
      const reqPath = path8.join(projectRoot, dependencyFile);
      const content = await fs6.promises.readFile(reqPath, "utf-8");
  ...
  async gatherRustInfo(projectRoot, dependencyFile) {
    try {
      const cargoPath = path8.join(projectRoot, dependencyFile);
      const content = await fs6.promises.readFile(cargoPath, "utf-8");
      const dependencies = [];
  ...
  async gatherGoInfo(projectRoot, dependencyFile) {
    try {
      const modPath = path8.join(projectRoot, dependencyFile);
      const content = await fs6.promises.readFile(modPath, "utf-8");
      const dependencies = [];
      content.split("\n").forEach((line) => {
...
import { autoInjectable as autoInjectable32 } from "tsyringe";

// src/services/LLM/context/MessageContextCleanup.ts
import { singleton as singleton14 } from "tsyringe";
...
import { autoInjectable as autoInjectable33 } from "tsyringe";
...
var _ActionsParser = class _ActionsParser {
  ...
  extractFilePath(tag) {
    ...
    return path9.resolve(process.cwd(), match[1]);
  }
  extractUrl(tag) {
...
import { autoInjectable as autoInjectable34, singleton as singleton15 } from "tsyringe";
...
import { inject as inject5, singleton as singleton16 } from "tsyringe";

// src/services/LLMProviders/OpenRouter/OpenRouterAPICostTracking.ts
import { autoInjectable as autoInjectable35 } from "tsyringe";
...
OpenRouterAPI = _ts_decorate48([
  singleton16(),
  _ts_param5(5, inject5(OpenRouterAPICostTracking)),
...
import { autoInjectable as autoInjectable36, container as container6, singleton as singleton17 } from "tsyringe";
...
var _LLMProvider = class _LLMProvider {
  ...
  initializeProvider(type) {
    ...
    switch (type) {
      case "open-router":
        provider = container6.resolve(OpenRouterAPI);
        break;
      default:
  ...
  static getInstance(type) {
    const provider = container6.resolve(_LLMProvider).getProvider(type);
    if (!provider) {
      throw new Error(`Unsupported provider type: ${type}`);
    }
    return provider;
...
import { autoInjectable as autoInjectable37 } from "tsyringe";
...
import { autoInjectable as autoInjectable38, singleton as singleton18 } from "tsyringe";
...
import { singleton as singleton19 } from "tsyringe";
...
var _Run = class _Run extends Command {
  constructor(argv, config4) {
    ...
    __publicField(this, "rl");
    this.configService = container7.resolve(ConfigService);
    this.modelManager = container7.resolve(ModelManager);
    this.streamHandler = container7.resolve(StreamHandler);
    this.openRouterAPI = container7.resolve(OpenRouterAPI);
    this.sessionManager = container7.resolve(InteractiveSessionManager);
  ...
  async run() {
    ...
    try {
      ...
      const agent = container7.resolve(CrackedAgent);
...
// src/index.ts
var crackedAgent = container8.resolve(CrackedAgent);
export {
  Run,
  crackedAgent
};
